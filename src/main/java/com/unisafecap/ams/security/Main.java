...
import com.unisafecap.ams.risk.model.dto.request.RiskLoanAccountDto;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		byte[] srcData = "{\"outTradeNo\":\"xxxxxx\",\"callbackURL\":\"http://www.baidu.com\",\"loanApplyDto\":{\"contractNo\": \"CP1273917239\"}}".getBytes(); // 源数据串
		RiskControlAuditDto audit = new RiskControlAuditDto();

		audit.setCallbackUrl("http://www.baidu.com");

		audit.setOutTradeNo("xxxxxx");

		RiskLoanApplyDto loanApply = new RiskLoanApplyDto();
		loanApply.setAssureFee(BigDecimal.valueOf(100.01));
		loanApply.setTrustProjectCode("111111");
		loanApply.setContractNo("CP1273917239");

		RiskLoanUserDto loanUser = new RiskLoanUserDto();
		loanUser.setCertId("411381198302177118");
		loanApply.setLoanUser(loanUser);

		List<RiskRelationUserDto> relationUsers = new ArrayList<RiskRelationUserDto>();
		RiskRelationUserDto relationUser = new RiskRelationUserDto();
		relationUser.setCertId("411381198602177118");
		relationUsers.add(relationUser);
		loanApply.setRelationUsers(relationUsers);

		RiskLoanAccountDto loanAccount = new RiskLoanAccountDto();

		loanAccount.setPutoutAccountType("11");
		loanApply.setLoanAccount(loanAccount);
